content = """# PROMPT DE HANDOVER E CONTINUIDADE DO PROJETO - MATRIZ JONATHA
...
5. **Restrições Geométricas Rígidas:**
   ...
   - Envelope externo cilíndrico escalonado ($\varnothing 93,00 \\times 69,90\\text{ mm} \\rightarrow \\varnothing 89,50 \\times 10,80\\text{ mm} \\rightarrow \\varnothing 79,50 \\times 28,30\\text{ mm}$) MANTIDO 100% IDÊNTICO à Matriz 2 para compatibilidade mecânica de montagem na máquina existente.
...
A Matriz 2 possuía um funil de entrada curto ($21,40\\text{ mm}$) que caía abruptamente de $\varnothing 75,60\\text{ mm}$ para uma fenda reta paralela de **$87,60\\text{ mm}$ de comprimento**.
...
1. **Entrada Cilíndrica Pura ($\varnothing 75,60\\text{ mm}$ a $Z=0$):** Mantém parede de aço robusta na face traseira.
...
with open("AUTO_PROMPT_CONTINUIDADE_MATRIZ_JONATHA.md", "w", encoding="utf-8") as f:
    f.write(content)

print("AUTO_PROMPT_CONTINUIDADE_MATRIZ_JONATHA.md gerado com sucesso!")
