...
def secao6(ic):
    ...
    return (
        ...
        f"piloto de centragem Ø{dim(corpo['piloto_traseiro']['Ø_mm'])} × "
        f"{dim(corpo['piloto_traseiro']['altura_mm'])} atrás do flange, "
        f"chanfro {dim(corpo['chanfro_corpo_flange']['cateto_mm'])} × "
        f"{dim(corpo['chanfro_corpo_flange']['angulo_graus'])}° na transição corpo→flange):\n\n"
...
**Modelo base (aprovado, continua oficial):** `07_CAD_Matrizes/Matriz_Jonatha_v27_OFICIAL/MatrizJonatha.step` — SSOT v27.0
