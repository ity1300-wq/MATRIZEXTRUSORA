...
Gerada por `04_Dados_SSOT_e_Scripts/auditar_step_correlacoes.py` (código de saída 1 se algo diverge, para entrar no portão). Cada número abaixo foi **medido no arquivo** na hora em que este relatório foi escrito, com `cadquery`, face por face: 64 arquivos STEP abertos. Status: **DIVERGENTE**.
