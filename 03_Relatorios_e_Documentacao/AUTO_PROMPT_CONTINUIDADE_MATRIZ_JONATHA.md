...
**Decisões já tomadas pelo usuário (não re-propor o que ele rejeitou):**
   ...
   * **Gedeon: o arquivo dele É a matriz certa**: A GEDEON CERTA E O ARQUIVO DELE, entregue como esta - nao re-escavar canal nem funil nele. A tentativa nossa de "Gedeon corrigida" (re-corte do canal historico no bloco) foi refutada por medida e APAGADA por inteiro em 2026-09-13, a pedido: gerador, JSON, relatorio, STEP e montagem. O que sobrou de valor saiu da prosa e virou medicao: gerar_gedeon_certa.py compara a Gedeon entregue com a certa nos proprios arquivos dela, e o unico defeito legítimo do arquivo dele (furos de pino como cavidade selada) e consertado so partidao no plano de particao, sem tirar aco.
   * **Organização do CAD: uma pasta por matriz**: UMA PASTA POR MATRIZ, sem camada intermediaria e sem pasta so com ponteiro: 07_CAD_Matrizes/Matriz_Jonatha_v27_OFICIAL, Matriz_Jonatha_v28_1_PROPOSTA, Matriz_Gedeon_Certa, Matriz_Gedeon_Entregue_HISTORICA, Matriz_Copo_HISTORICA, Matriz_Desenvolvimento_HISTORICA. Cada uma contem os STEP dela e um README. O que o cabeçote nao e matriz fica em 06_CAD_Cabecote_EX-030/STEP/.
   * **Método de auditoria (v1.0 encerrado; novo a desenvolver)**: O METODO DE AUDITORIA v1.0 (proposta -> PR -> veredito -> re-selo do baseline) esta ENCERRADO, nao suspenso: nao reabrir veredito das PRs abertas, nao rodar --atualizar-hashes, nao editar 05_Interface_Auditoria/. Um metodo novo vai ser desenvolvido depois, com ele. Ate la, a garantia de consistencia e o portao verificar_cadeia.py (cadeia completa + pares documento x JSON + regras 1 e 2 por conteudo).
   * **Sessão no GitHub e tamanho do workspace**: O ESTADO DA SESSAO MORA NO REPO, nao na conversa: CONTINUIDADE.md na raiz, com README apontando para ele, e o branch padrao main igual ao trabalho de continue. O workspace foi enxugado pelo que e recriavel: PNG de rascunho fora, o DXF do croquis compactado dentro do repo, objetos soltos do git empacotados por gc. As capturas dele (uploads/) ficam FORA do github porque o repo e publico.
...
## 6. MAPA DE ARQUIVOS
- `07_CAD_Matrizes/Matriz_Jonatha_v27_OFICIAL/MatrizJonatha*  (o caminho antigo segue valido: ha um atalho em `01_CAD_MatrizJonatha_Oficial/` para o mesmo arquivo).step` — v27.0 aprovado (intocado)
- `07_CAD_Matrizes/Matriz_Jonatha_v28_1_PROPOSTA/MatrizJonatha_v28*.step` — proposta DFM (corpo A/B, canal 1 sólido,
...
- `04_Dados_SSOT_e_Scripts/medir_perfis_matrizes_x_cabecote.py` →
